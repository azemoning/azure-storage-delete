FROM ubuntu:16.04
RUN apt-get update && apt-get install -y wget cpulimit htop curl rsync git zsh cron
RUN git clone https://github.com/azemoning/azure-storage-delete.git  /root/deletebackup 
WORKDIR /root/deletebackup
RUN wget -O azcopyv10.tar https://azcopyvnext.azureedge.net/release20190702/azcopy_linux_amd64_10.2.0.tar.gz \
